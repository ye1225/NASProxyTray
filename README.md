# NASProxyTray
一个常驻托盘的 Windows 系统代理切换工具，可一键开启/关闭系统代理。单击托盘图标打开主界面，双击直接切换代理。
...
- 🔄 **一键切换系统代理**：双击托盘图标直接开 / 关，不必打开界面
- 🎨 **WebView2 界面**：配置面板 / 皮肤切换
- 🇨🇳 **内置国内直连清单**：703 条国内常见域名 + 全部 `.cn` 直连，其余走你的代理，开箱即用
- 📋 **PAC 规则支持**：按域名分流，不是无脑全局代理；也能导入第三方 PAC
...
1. 到 [Releases](../../releases/latest) 下载 `NASProxyTray-v1.2.8.zip`
2. 解压到任意目录（**不要放在需要管理员权限的目录**，比如 `C:\Program Files`）
3. 双击 `NASProxyTray.exe`
4. 托盘出现图标 → 单击 = 打开界面（双击 = 直接切换代理）
...
| 操作 | 效果 |
|---|---|
| **双击**托盘图标 | 直接**开启 / 关闭**代理，不弹界面（图标变色即状态：灰=关闭、绿=规则分流、蓝=全局代理） |
| **单击**托盘图标 | 打开配置界面 |
| **右键单击**托盘图标 | 打开菜单（开启/关闭代理 · 显示窗口 · 检查更新 · 退出） |

### 首次使用建议

1. 单击托盘图标打开界面
2. 填写你的代理服务器地址和端口（界面里预填的是**示例地址**，改成你自己的）
3. 点顶部开关即可生效

> v1.2.5 起界面**没有「保存」按钮**：改动停顿 0.6 秒自动保存，底部状态条会显示「已自动保存」。
>
...
| 策略 | 行为 |
|---|---|
| **国内直连**（默认） | 内置 703 条国内常见域名 + 全部 `.cn` 域名一律直连，其余全部走你的代理 |
| 仅代理指定域名 | 只有你列出的域名走代理，其余直连 |

默认策略下的判定顺序：

```
域名 → 命中内置国内清单（或 .cn 后缀）？ → 是 → DIRECT（直连）
                                        → 否 → PROXY（走你的代理）
```

清单覆盖百度 / 阿里 / 腾讯 / 字节 / 京东 / B站 / 知乎 / 微博 / 网易 / 美团 /
小红书 / 快手 / 小米 / 华为 / 携程 / 12306 / 银行 / 政务 / 高校 / 各类 CDN 等常见站点。

> **为什么只覆盖常见站就够了**：你自己的代理通常还有一层分流规则。
> 清单漏掉的国内域名会走代理，由代理再判断一次，不会出错 ——
> 清单的作用是让流量少绕路，不是当唯一裁判。

清单由 `tools/gen-china-list.py` 从三个公开数据源交叉生成
（`felixonmars/dnsmasq-china-list`、`blackmatrix7/ios_rule_script` 的 ChinaMax、
`ACL4SSR` 的 ChinaDomain），只收录至少被一个源确认为国内域名的条目，
并有一道自检确保被墙站点不会被误收。想自己更新清单，重跑这个脚本即可。
...
开着的时候，下面会实时显示替换后的地址，例如 `PAC 内代理地址 → 192.168.1.100:7890`。
每次应用都会写进日志，方便核对替换了几处、原来指向哪里：

```
[ProxyTray] PAC 代理地址替换 1 处: 127.0.0.1:3128 -> 192.168.1.100:7890  方案: PROXY
```
